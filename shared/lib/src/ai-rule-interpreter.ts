...
export class AIRuleInterpreter {
   ...
   */
  async evaluateRules(rules: AIRule[], context: ExecutionContext): Promise<InterpretationResult> {
  ...
  private async evaluateRule(
    ...
    const confidence =
      totalWeight > 0 ? (conditionsMet / totalWeight) * rule.metadata.confidence : 0;
    ...
    reasoning.push(
      `Rule ${rule.name} (${rule.type}): ${conditionsMet}/${totalWeight} conditions met, confidence: ${(confidence * 100).toFixed(1)}%`
    );
