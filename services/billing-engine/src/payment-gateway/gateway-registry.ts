...
export type GatewayName = 'stripe' | 'paypal' | 'razorpay' | 'adyen' | 'square' | 'alipay' | 'wechatpay' | 'mpesa';
...
export class GatewayRegistry {
  ...
  private loadGatewayConfig(): Record<GatewayName, GatewayConfig> {
    ...
    return {
      stripe: {
        ...
        webhookSecret: isProduction ? process.env.STRIPE_WEBHOOK_SECRET : process.env.STRIPE_WEBHOOK_SECRET_TEST,
        publishableKey: isProduction ? process.env.STRIPE_PUBLISHABLE_KEY : process.env.STRIPE_PUBLISHABLE_KEY_TEST,
        apiVersion: '2023-10-16',
        supportedCurrencies: ['usd', 'eur', 'gbp', 'jpy', 'aud', 'cad', 'chf', 'cny', 'inr', 'brl', 'mxn', 'sgd', 'hkd', 'nzd', 'sek', 'dkk', 'nok', 'pln', 'czk', 'huf', 'thb', 'myr', 'idr', 'php', 'vnd', 'zar', 'aed', 'sar', 'try', 'rub', 'ils', 'krw', 'twd'],
        supportedCountries: ['US', 'GB', 'EU', 'CA', 'AU', 'JP', 'SG', 'HK', 'NZ', 'IN', 'BR', 'MX', 'ZA', 'AE', 'SA', 'TR', 'IL', 'KR', 'TW', 'CN', 'TH', 'MY', 'ID', 'PH', 'VN'],
      ...
      paypal: {
        ...
        name: 'PayPal',
        clientId: isProduction ? process.env.PAYPAL_CLIENT_ID : process.env.PAYPAL_CLIENT_ID_SANDBOX,
        clientSecret: isProduction ? process.env.PAYPAL_CLIENT_SECRET : process.env.PAYPAL_CLIENT_SECRET_SANDBOX,
        mode: isProduction ? 'live' : 'sandbox',
        supportedCurrencies: ['usd', 'eur', 'gbp', 'jpy', 'aud', 'cad', 'chf', 'cny', 'hkd', 'sgd', 'nzd', 'inr', 'mxn', 'brl', 'php', 'pln', 'sek', 'dkk', 'nok', 'thb', 'vnd', 'myr', 'idr'],
        supportedCountries: ['US', 'GB', 'EU', 'CA', 'AU', 'JP', 'SG', 'HK', 'NZ', 'IN', 'BR', 'MX', 'PH', 'PL', 'SE', 'DK', 'NO', 'TH', 'VN', 'MY', 'ID'],
        requires3DSecure: true,
      ...
      adyen: {
        ...
        supportedCurrencies: ['usd', 'eur', 'gbp', 'jpy', 'aud', 'cad', 'chf', 'cny', 'hkd', 'sgd', 'nzd', 'inr', 'brl', 'mxn', 'pln', 'sek', 'dkk', 'nok', 'czk', 'huf', 'ron', 'bgn', 'hrk', 'isk'],
        supportedCountries: ['US', 'GB', 'EU', 'CA', 'AU', 'JP', 'SG', 'HK', 'NZ', 'IN', 'BR', 'MX', 'PL', 'SE', 'DK', 'NO', 'CZ', 'HU', 'RO', 'BG', 'HR', 'IS'],
        requires3DSecure: true,
  ...
  selectOptimalGateway(context: {
    ...
    if (context.userPreferences && context.userPreferences.length > 0) {
      const preferred = enabledGateways.find(g => 
        context.userPreferences!.includes(g)
      );
  ...
  private estimateFee(gateway: GatewayName, amount: number): number {
    const feeStructures: Record<GatewayName, { fixed: number; percentage: number }> = {
      ...
      paypal: { fixed: 0.30, percentage: 2.99 },
      razorpay: { fixed: 0, percentage: 2 },
      adyen: { fixed: 0.25, percentage: 2.5 },
      square: { fixed: 0.10, percentage: 2.6 },
