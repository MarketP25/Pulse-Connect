...
async function savePolicyToPersistence(persistence: any, policy: Policy) {
  ...
  const idx = existing.findIndex(
    (x: Policy) => x.policyId === policy.policyId && x.version === policy.version
  );
  if (idx !== -1) existing[idx] = policy;
...
async function run() {
  const argv = yargs(hideBin(process.argv))
    .command("create [file]", "Create and sign a policy", (y) =>
      y.positional("file", { type: "string", describe: "JSON file with policy object" })
    )
    .command("deprecate", "Deprecate an existing policy", (y) =>
      y
