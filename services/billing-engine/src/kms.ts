...
class AzureKeyVaultAdapter implements KMSAdapter {
  ...
  constructor(vaultUrl: string, keyName: string) {
    ...
    try {
      ...
      throw new Error(
        "Azure SDK not installed or misconfigured. Install @azure/identity and @azure/keyvault-keys to use Azure Key Vault adapter"
      );
    }
  }
...
export class MARPKV {
  ...
  constructor() {
    ...
    if (process.env.MARP_AWS_KMS_KEY_ID) {
      ...
      this.adapter = new AzureKeyVaultAdapter(
        process.env.MARP_AZURE_KEY_VAULT_URL,
        process.env.MARP_AZURE_KEY_NAME
      );
    } else {
