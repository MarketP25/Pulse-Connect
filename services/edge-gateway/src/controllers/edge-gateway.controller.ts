...
export class EdgeGatewayController {
   ...
   */
  @Post('brand/support')
  async ingestBrandSupport(@Body() request: BrandSupportRequestDto, @Headers() headers: Record<string, string>) {
  ...
  async getPrivacy() {
    ...
    const privacyEmailMatch = content.match(/Privacy (Contact )?Email:\s*(.*)/i) || content.match(/Contact Email:\s*(.*)/i);
    ...
    return {
      version: hash.slice(0, 12),
      effectiveDate: effectiveDateMatch ? effectiveDateMatch[1].trim() : null,
      privacyEmail: privacyEmailMatch ? (privacyEmailMatch[2] || privacyEmailMatch[1]).trim() : null,
      contentMarkdown: content,
      hash,
    };
  }
  ...
  async getGovernanceCharter() {
    ...
    try {
      ...
      content = '# Pulsco Platform Governance & Enforcement Charter\n\nSee /governance-charter in the portal.';
    }
    const hash = crypto.createHash('sha256').update(content).digest('hex');
    return {
      version: hash.slice(0, 12),
