...
export class ChatbotAdapter implements SubsystemAdapter {
  ...
  private hasContradictions(response: string): boolean {
    ...
    return contradictions.some(
      ([a, b]) => response.toLowerCase().includes(a) && response.toLowerCase().includes(b)
    );
  }
  ...
  private getSafeFallbackResponse(reason: string): string {
    const fallbacks = {
      unsafe_content:
        "I'm sorry, but I can't assist with that request as it may violate our safety guidelines.",
      blocked_intent:
        "I'm not able to help with that type of request. Is there something else I can assist you with?",
