...
export class PlanetaryLocalizationIntegration {
  ...
  private readonly PLANETARY_DATA = {
    continents: {
      Africa: {
        languages: ["sw", "am", "ha", "yo", "ig"],
        timezones: ["Africa/Nairobi", "Africa/Lagos", "Africa/Johannesburg"]
      },
      Asia: {
        languages: ["zh", "hi", "ar", "ja", "ko"],
        timezones: ["Asia/Shanghai", "Asia/Tokyo", "Asia/Dubai"]
      },
      Europe: {
      ...
      "South America": {
        languages: ["es", "pt", "qu"],
        timezones: ["America/Sao_Paulo", "America/Buenos_Aires", "America/Lima"]
      },
  ...
  async getLocalizationContext(
    ...
    if (context.userLocation) {
      const geocodeResult = await this.proximityService.reverseGeocode(context.userLocation, {
        actorId,
  ...
  async getPlanetaryTimeContext(
    ...
    if (userLocation) {
      const geocodeResult = await this.proximityService.reverseGeocode(userLocation, {
        actorId,
        subsystem: "localization",
        purpose: "localization",
  ...
  private determineLocaleHierarchy(
    ...
    const language =
      context.userLanguage || regionInfo?.language || context.deviceLocale?.split("-")[0] || "en";

    const countryCode = regionInfo?.countryCode || context.deviceLocale?.split("-")[1] || "US";
    ...
    const currency =
      regionInfo?.currency || this.PLANETARY_DATA.countries[countryCode]?.currency || "USD";
  ...
  private calculatePlanetaryTimeInsights(localTime: Date): {
    ...
    for (let h = 0; h < 24; h++) {
      globalTimeDistribution[h.toString()] = Math.round(
        baseUsers * this.getHourActivityMultiplier(h)
      );
    }
  ...
  private async estimateLatency(
    ...
    const distance = await this.proximityService.distanceKm(userLocation, dataCenterCoords, {
      actorId,
      subsystem: "localization",
      purpose: "localization",
