...
const SignupSchema = z
  .object({
    ...
    role: z.enum(ROLES),
    language: z.string().refine((l) => isSupportedLanguage(l), {
      message: "Select a valid language"
    }),
    region: z.string().refine((r) => isSupportedRegion(r), {
...
export default function SignupPage() {
  ...
  const { setLanguage, setRegion } = useLanguage();
  const t = useT();
              ...
              >
                ...
                <div className="h-2 bg-gray-200 rounded mt-1 overflow-hidden">
                  <motion.div
                    ...
                    animate={{ width: `${Math.min(100, PASSWORD.length * 12)}%` }}
                  />
                </div>
                {errors.password && (
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {ROLES.map((role) => (
                        <div
                          ...
                          className={`cursor-pointer border rounded-lg p-4 text-center ${field.value === role ? "border-indigo-600 bg-indigo-100" : "border-gray-300"}`}
                        >
              ...
              >
                ...
                <div className="flex justify-between mt-4">
                  ...
                  <button
                    type="button"
                    onClick={() => setStep(3)}
                    disabled={!watch("language") || !watch("region")}
            ...
            {step === 4 && (
              ...
              >
                <h2 className="text-2xl font-bold">
                  {t("auth.welcome")},{" "}
                  {watched.role && watched.role.charAt(0).toUpperCase() + watched.role.slice(1)}!
                </h2>
                <p className="text-gray-600">
                  Redirecting to login in <strong>{countdown}</strong>…
                </p>
                <button
                  onClick={() => router.push(`/login?email=${encodeURIComponent(watched.email)}`)}
