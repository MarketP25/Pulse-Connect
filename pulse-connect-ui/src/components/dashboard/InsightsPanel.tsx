...
export function InsightsPanel({ title, recommendations, alerts }: Props) {
  return (
    <SectionCard title={title} subtitle="PULSCO AI + CSI recommendations, fraud alerts, and optimization signals.">
      <div className="grid gap-4 lg:grid-cols-2">
          ...
          {recommendations.map((item) => (
            <article key={item.id} className="rounded-lg border border-slate-200 p-3">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="rounded bg-slate-100 px-2 py-0.5 text-xs text-slate-700">{item.source}</span>
              </div>
        ...
        <div className="space-y-3">
          ...
          {alerts.map((alert) => (
            <article key={alert.id} className={`rounded-lg border p-3 ${alertTone[alert.severity]}`}>
