import { evaluateEmergencyProtocolBlock } from "./emergency-policy";

const REASON_CODE = "CSI_GATEWAY_ACCESS";
const HIGH_RISK_ACTION_RE =
  /(delete|remove|revoke|refund|charge|transfer|cancel|deprecate|shutdown|destroy)/i;
...
function isHighRiskAction(payload: Record<string, unknown>) {
  ...
  return (
    HIGH_RISK_ACTION_RE.test(action) ||
...
export async function proxyEdgeExecute(request: Request, appId: string) {
  ...
  const emergencyDecision = await evaluateEmergencyProtocolBlock(request, payload);
  if (emergencyDecision.blocked) {
    return json(
      {
        error: "EMERGENCY_PROTOCOL_ACTIVE",
        reason_code: REASON_CODE,
        message:
          emergencyDecision.message || "This action is blocked by the global emergency protocol.",
        blocked_control: emergencyDecision.control,
        emergency_protocol: emergencyDecision.snapshot
          ? {
              protocol_id: emergencyDecision.snapshot.protocolId,
              severity: emergencyDecision.snapshot.severity,
              activated_at: emergencyDecision.snapshot.activatedAt,
              risk_score: emergencyDecision.snapshot.riskScore,
              anomalies_observed: emergencyDecision.snapshot.anomaliesObserved,
              correlation_id: emergencyDecision.snapshot.correlationId,
              version: emergencyDecision.snapshot.version
            }
          : undefined
      },
      423
