...
function sanitizeEvent(input: unknown): TelemetryEvent | null {
  ...
  const ts =
    typeof input.ts === "number" && Number.isFinite(input.ts) ? Math.floor(input.ts) : Date.now();
  ...
  if (isPlainObject(input.meta)) {
    const forbiddenKeyRe =
      /(token|auth|authorization|cookie|password|secret|wallet|billing|admin|edge|marp)/i;
