...
function isLocalCriticalApisAllowed() {
  return process.env.NODE_ENV !== 'production' || process.env.PULSCO_ALLOW_LOCAL_CRITICAL_APIS === 'true'
}
...
function disabledPayload(input: CriticalActionGuardInput) {
  return {
    ...
    message: 'This local critical endpoint is disabled in production. Use Edge Gateway /edge/execute.',
