...
async function fetchTerms() {
  ...
  if (process.env.EDGE_GATEWAY_URL) candidates.push(`${process.env.EDGE_GATEWAY_URL.replace(/\/$/, '')}/legal/terms`);
  if (process.env.NEXT_PUBLIC_EDGE_GATEWAY_URL) candidates.push(`${process.env.NEXT_PUBLIC_EDGE_GATEWAY_URL.replace(/\/$/, '')}/legal/terms`);
...
export default async function TermsPage() {
  ...
  return (
    <main className="mx-auto max-w-4xl px-4 py-12 prose prose-invert">
      ...
      <p>
        <strong>Legal Entity:</strong> {legalEntity || '[Legal Entity Name]'}<br />
        <strong>Governing Law Jurisdiction:</strong> {governingLaw || '[Governing Law Jurisdiction]'}<br />
      ...
      <pre className="whitespace-pre-wrap leading-relaxed text-sm sm:text-base">{contentMarkdown || '# Terms unavailable. Please try again later.'}</pre>
    </main>
  );
}
