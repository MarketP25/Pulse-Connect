...
export default async function PrivacyPage() {
  ...
  if (process.env.EDGE_GATEWAY_URL) candidates.push(`${process.env.EDGE_GATEWAY_URL.replace(/\/$/, '')}/legal/privacy`);
  if (process.env.NEXT_PUBLIC_EDGE_GATEWAY_URL) candidates.push(`${process.env.NEXT_PUBLIC_EDGE_GATEWAY_URL.replace(/\/$/, '')}/legal/privacy`);
  ...
  return (
    <main className="mx-auto max-w-4xl px-4 py-12 prose prose-invert">
      ...
      <pre className="whitespace-pre-wrap leading-relaxed text-sm sm:text-base">{contentMarkdown || '# Privacy Policy unavailable. Please try again later.'}</pre>
    </main>
  );
}
